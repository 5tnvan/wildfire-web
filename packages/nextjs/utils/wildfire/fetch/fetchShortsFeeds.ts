...
export const fetchAllApproved = async (from: number, to: number) => {
  const supabase = createClient();
...
export const fetchRandomFeed = async (limit: any) => {
  ...
  const { data } = await supabase
    ...
    .select(
      ...
      created_at, country:country_id(id, name),
...
export const fetchVideoAndRandomFeed = async (video_id: any, limit: any) => {
  ...
  const { data: data1 } = await supabase
    ...
    .select(
      ...
      created_at, country:country_id(id, name), 
      profile:user_id(id, username, avatar_url, wallet_id), 
      3sec_tips(id, created_at, network, transaction_hash, amount, currency, comment, tipper:wallet_id(id, username, avatar_url)), 
      3sec_views(view_count), 3sec_fires(count), 
      3sec_comments(*, profile:user_id(id, username, avatar_url), 
      3sec_replies:id(*, profile:user_id(id, username, avatar_url)))`,
    )
    .eq("id", video_id)
    .single();
...
export const fetchWithin48Hrs = async (limit: any) => {
  ...
  const { data, error } = await supabase
    ...
    .select(
      ...
      created_at, country:country_id(id, name), 
      profile:user_id(id, username, avatar_url, wallet_id), 
      3sec_tips(created_at, network, transaction_hash, amount, currency, comment, 
      tipper:wallet_id(id, username, avatar_url)), 3sec_views(view_count), 3sec_fires(count), 
      3sec_comments(*, profile:user_id(id, username, avatar_url), 3sec_replies:id(*, profile:user_id(id, username, avatar_url)))`,
      { count: "exact" },
    )
...
export const fetchUserShortsFeedWithRange = async (user_id: string, from: any, to: any) => {
  ...
  const { data } = await supabase
    ...
    .select(
      `id, thumbnail_url, 
      video_url, playback_id, 
      created_at, country:country_id(name), 
      profile:user_id(id, username, avatar_url, wallet_id), 
      3sec_views(view_count), 3sec_fires(count), 
      3sec_comments(*, profile:user_id(id, username, avatar_url), 3sec_replies:id(*, profile:user_id(id, username, avatar_url)))`,
    )
...
export const fetchUserFeedFromArrayOfFollowing = async (followingArray: any, from: number, to: number) => {
  ...
  const { data } = await supabase
    ...
    .select(
      ...
      video_url, playback_id, 
      created_at, country:country_id(name), 
      profile:user_id(id, username, avatar_url, wallet_id), 3sec_views(view_count),
      3sec_tips(created_at, network, transaction_hash, amount, currency, comment, tipper:wallet_id(id, username, avatar_url)), 
