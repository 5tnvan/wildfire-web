...
export async function updateFireRead(notification_id: any) {
  ...
  const { data, error } = await supabase.from("notifications_fires").update({ read: true }).eq("id", notification_id);
  if (error) {
    return null;
  }
  return data;
...
export async function updateTipRead(notification_id: any) {
  ...
  const { data, error } = await supabase.from("notifications_tips").update({ read: true }).eq("id", notification_id);
  if (error) {
    console.log(error);
    return null;
  }
