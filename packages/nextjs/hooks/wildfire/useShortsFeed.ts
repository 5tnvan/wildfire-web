...
import {
  ...
  fetchRandomFeed,
  fetchWithin48Hrs,
} from "~~/utils/wildfire/fetch/fetchShortsFeeds";
 ...
 **/
export const useShortsFeed = (filter: any, limit: any) => {
  ...
  const fetchFeed = async () => {
    try {
      ...
      switch (filter) {
        case "within48hrs":
          data = await fetchWithin48Hrs(limit);
          break;
        case "latestTipped":
          data = await fetchLatestTipped(limit);
          break;
        case "mostViewed":
          data = await fetchMostViewed(limit);
          break;
        default:
